        ...
        <div class="StudentListDiv">
            <Table :columns="columns" :data="data" border>
                ...
                <template slot="sno" slot-scope="{ row, index }">
                    <Input type="text" v-if="editIndex === index" v-model="Edit_Student_sno"/>
                    <span v-else>{{ row.sno }}</span>
                </template>

                <template slot="sname" slot-scope="{ row, index }">
                    <Input type="text" v-if="editIndex === index" v-model="Edit_Student_sname"/>
                    <span v-else>{{ row.sname }}</span>
                </template>

                <template slot="gender" slot-scope="{ row, index }">
                    <Input type="text" v-if="editIndex === index" v-model="Edit_Student_gender"/>
                    <span v-else>{{ row.gender }}</span>
                </template>

                <template slot="birth" slot-scope="{ row, index }">
                    <Input type="text" v-if="editIndex === index" v-model="Edit_Student_birth"/>
                    <span v-else>{{ getBirthday(row.birth) }}</span>
                </template>

                <template slot="phone" slot-scope="{ row, index }">
                    <Input type="text" v-if="editIndex === index" v-model="Edit_Student_phone"/>
                    <span v-else>{{ row.phone }}</span>
                </template>


                <template slot="cname" slot-scope="{ row, index }">
                    <Input type="text" v-if="editIndex === index" v-model="Edit_Student_cname"/>
                    <span v-else>{{ row.cname }}</span>
                </template>


                <template slot="classroom" slot-scope="{ row, index }">
                    <Input type="text" v-if="editIndex === index" v-model="Edit_Student_classroom"/>
                    <span v-else>{{ row.classroom }}</span>
                </template>
                 <template slot="pwd" slot-scope="{ row, index }">
                    <Input type="text" v-if="editIndex === index" v-model="Edit_Student_pwd"/>
                    <span v-else>{{ row.pwd }}</span>
                </template>
                <template slot="action" slot-scope="{ row, index }">
                    <div>
                        <div class="templateBtn" v-if="editIndex === index">
                            <Button @click="handleSave(index)" ghost size="small" type="primary">保存</Button>
                            <Button @click="editIndex = -1" ghost size="small" type="error">取消</Button>
                        </div>
                        <div class="templateBtn" v-else>
                            <Button @click="handleEdit(row, index)" size="small" type="primary">操作</Button>
                            <Button @click="DeleteEdit(row, index)" size="small" type="error">删除</Button>
                        </div>
                    </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleSave(index) {
        ...
        this.data[index].pwd = this.Edit_Student_pwd;


        axios.post("/api/fixstudentAndclassdata", this.data[index])
          .then((res) => {
            console.log(res);
      ...
      DeleteEdit(row, index){
        ...
        axios.post("/api/studentAndclassdataDelete", obj)
          .then((res) => {
            if (res.data.message === "OK") {
