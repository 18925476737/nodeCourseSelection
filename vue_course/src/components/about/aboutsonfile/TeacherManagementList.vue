<template>
    <div class="TeacherManagementList">
        ...
        <div class="TeacherManagementListDiv">
            <Table :columns="columns" :data="data" border>
                ...
                <template slot-scope="{ row, index }" slot="pwd">
                    <Input type="text" v-model="Edit_Teacher_pwd" v-if="editIndex === index"/>
        ...
        <div class="TeacherManagementListPage">
            <Page :total="total" :page-size="pageSize" :current="pageCurrent" size="small" :page-size-opts="[5,10,15]" show-elevator show-sizer @on-change="onChangetable"/>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onChangetable(e){
            console.log(e)
        axios.get("/api/teacherdata?pageSize="+this.pageSize+"&offset="+this.pageSize *(e-1))
          .then((res) => {
            if (res.data) {
              this.data = res.data.result;
            }
      ...
      handleSave(index) {
        ...
        this.data[index].pwd = this.Edit_Teacher_pwd;
        this.data[index].remark = this.Edit_Teacher_remark;


        axios.post("/api/fixteacherdata", this.data[index])
          .then((res) => {
            console.log(res);
      ...
      DeleteEdit(row, index){
        ...
        axios.post("/api/TeacherDelete", obj)
          .then((res) => {
            console.log(res.data);
            if (res.data.message === "OK") {
