        ...
        <div class="CourseListDiv">
            <Table :columns="columns" :data="data" border>
                ...
                <template slot-scope="{ row, index }" slot="courseTeacherphone">
                    <Input type="text" v-model="Edit_course_tphone" v-if="editIndex === index"/>
                ...
                <template slot-scope="{ row, index }" slot="action">
                    <div>
                        <div v-if="editIndex === index" class="templateBtn">
                            <Button type="primary" ghost size="small" @click="handleSave(index)">保存</Button>
                            <Button type="error" ghost size="small" @click="editIndex = -1">取消</Button>
                        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getHomeInfo() {
        axios.get("/api/teacherAndCourse")
      ...
      handleSave(index) {
        ...
        axios.post("/api/fixteacherAndCoursedata", this.data[index])
          .then((res) => {
            console.log(res);
      ...
      DeleteEdit(row, index){
        ...
        axios.post("/api/teacherAndCoursedataDelete", obj)
          .then((res) => {
            console.log(res.data);
             if (res.data.message === "OK") {
