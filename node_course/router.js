...
router.post('/Getlogin', function (req, res, next) {
    ...
    if (identity === 'Administrator') {
       let searchbynamesql = 'SELECT * FROM t_admin\n' +
            'WHERE t_admin.loginname=?\n' +
            'AND t_admin.pwd=?\n'
        let searchbynameParams = [name,  password]
        ...
        let  searchbynamesql = 'SELECT * FROM t_teacher\n' +
        ...
        let  searchbynamesql = 'SELECT * FROM t_student\n' +
...
router.post('/Addteacherdata', function (req, res, next) {
    ...
    db.query('select * from t_teacher', [], function (result, fields) {
        ...
        let addSql = 'INSERT INTO t_teacher(tno,loginname,tname,pwd,phone,hiredate,remark,email) VALUES(?,?,?,?,?,?,?,?)';
        let addSqlParams = [tno, body.username, body.name, body.password, body.contactway, body.data, body.desc, body.mail,];
...
router.post('/fixteacherdata', function (req, res, next) {
    ...
    let updatasql = 'UPDATE t_teacher SET loginname = ?,tname =?,pwd=?,phone=?,hiredate =?,remark=?,email=? WHERE tno = ?';
    let updataSqlParams = [loginname, tname, pwd, phone, hiredate, remark, email, tno];
    db.query(updatasql, updataSqlParams, (err, results) => {
        if (err) {
            console.log(err);
        }
...
router.post('/addscore', function (req, res, next) {
    ...
    let updatasql = 'UPDATE t_student_course SET score = ? WHERE sno = ? and cno = ?';
    let updataSqlParams = [score, sno, cno];
    db.query(updatasql, updataSqlParams, (err, results) => {
        if (err) {
            console.log(err);
        }
...
router.get('/getTeacher', function (req, res, next) {
    ...
    let selectsql = 'select * from  t_teacher'
...
router.post('/AddteacherAndCourse', function (req, res, next) {
    ...
    db.query('select * from t_course', [], function (result, fields) {
        ...
        db.query(findTeachersql, findTeacherSqlParams, (result) => {
            ...
            let addSql = 'INSERT INTO t_course(cno,cname,credit,startDate,endDate,tno) VALUES(?,?,?,?,?,?)';
...
router.post('/fixteacherAndCoursedata', function (req, res, next) {
    ...
    db.query(fixTeachersql, fixTeacherSqlParams, (result) => {
        ...
        if (result.length > 0) {
            ...
            let updataCoursesql = 'UPDATE t_course SET cname=?,credit=?,startDate=?,endDate=?,tno=? WHERE cno = ?';
...
router.get('/StudentAndClass', function (req, res, next) {
    let selectsql = 'select * from  t_student s,t_class c WHERE s.classno = c.classno'
    db.query(selectsql, [], function (result, fields) {
        return res.status(200).json({
            err_code: 0,
            message: 'OK',
...
router.post('/AddStudentAndClass', function (req, res, next) {
    ...
    db.query('select * from t_student', [], function (result, fields) {
        ...
        db.query(findClasstsql, findClassSqlParams, (result) => {
            ...
            let addStudentSql = 'INSERT INTO t_student(sno,pwd,sname,phone,gender,birth,classno,remark,email) VALUES(?,?,?,?,?,?,?,?,?)';
            let addStudentSqlParams = [sno, PwdOfstudent, NameOfstudent, ContactMethod, gender, DateOfBirth, classno, desc, mail];
...
router.post('/fixstudentAndclassdata', function (req, res, next) {
    ...
    db.query(fixStudentsql, fixStuentSqlParams, (result) => {
        ...
        if (result.length > 0) {
            ...
            let updataCoursesql = 'UPDATE t_student SET pwd=?,sname=?,phone=?,gender=?,birth=?,classno=?,remark=?,email=? WHERE sno = ?';
            let updataCourseSqlParams = [pwd,sname,phone,gender,birth,classno,remark,email,sno];
...
router.post('/TeacherFindCourse', function (req, res, next) {
    ...
    let TeacherFindCoursesql = 'SELECT * FROM t_teacher ,t_course \n' +
        'WHERE t_teacher.tno = t_course.tno \n' +
        'AND t_teacher.loginname = ?'
    let TeacherFindCourseSqlParams = [loginname]

    console.log(TeacherFindCoursesql)
...
//退课tuike
router.post('/tuike', function (req, res, next) {
    let deleteSql = 'DELETE FROM t_student_course  WHERE sno = ? and cno = ?';
    let deleteSqlParams = [req.body.sno,req.body.cno];
    db.query(deleteSql, deleteSqlParams, (err, results) => {
        if (err) {
            console.log(err);
        }
...
//查询报名某门课程的学生信息
router.post('/getStudengByCourse', function (req, res, next) {
    let SqlParams = [req.body.cno];
    let selectsql = 'select * from  t_student_course s,t_course c,t_student t WHERE s.cno = c.cno AND s.cno = ? and t.sno = s.sno'
    db.query(selectsql, SqlParams, function (result, fields) {
        return res.status(200).json({
            err_code: 0,
            message: 'OK',
            result: result
